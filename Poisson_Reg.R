...
# Description: Implement zero-inflated Poisson Regression in JAGS and output
...
model{
  ...
  for (i in 1:n) {
    ...
    # Poisson Regression Model for HomeTeam 
    
    y_homegoals[i] ~ dpois(mu_home[i])
    pred_home[i] ~ dpois(mu_home[i]) # Create another lambda_home with newer dataset for prediction next year
    mu_home[i] <- lambda_home[i]*z_home[i] + 0.00001

    z_home[i] ~ dbern(psi)
    # log(lambda_home[i]) <- beta0 + beta1 * Xhome[i,1] + beta2 * Xhome[i,2] + beta3 * Xhome[i,3] + beta4 * Xhome[i,4] + beta5 * Xhome[i,5] + beta6 * Xhome[i,6]
    lambda_home[i] <- exp(beta0 + beta1 * Xhome[i,1] + beta2 * Xhome[i,2] + beta3 * Xhome[i,3] + beta4 * Xhome[i,4] + beta5 * Xhome[i,5] + beta6 * Xhome[i,6])
    # Poisson Regression Model for AwayTeam 
    y_awaygoals[i] ~ dpois(mu_away[i])
    pred_away[i] ~ dpois(mu_away[i]) # Create another lambda_away with newer dataset for prediction next year
    mu_away[i] <- lambda_away[i]*z_away[i] + 0.00001
    

    # log(lambda_away[i]) <- beta0 + beta1 * Xaway[i,1] + beta2 * Xaway[i,2] + beta3 * Xaway[i,3] + beta4 * Xaway[i,4] + beta5 * Xaway[i,5] + beta6 * Xaway[i,6]
    lambda_away[i] <- exp(beta0 + beta1 * Xaway[i,1] + beta2 * Xaway[i,2] + beta3 * Xaway[i,3] + beta4 * Xaway[i,4] + beta5 * Xaway[i,5] + beta6 * Xaway[i,6])
    z_away[i] ~ dbern(psi)
  ...
  # Priors for Poisson regression coefficients
  ...
  # Priors for Zero-Inflated Poisson regression
  psi ~ dunif(0, 0.7)

  # Calculate Deviance for HomeTeam bayesian Poisson regression model
  dev_home  <-  sum(devres_home[])
  
  # Calculate Deviance for AwayTeam bayesian Poisson regression model
...
model.inits <- list(beta0 = 1, beta1 = 0, beta2 = 0, beta3 = 0, beta4 = 0, beta5 = 0, beta6 = 0, psi = 0,
                    psi0 = 0.5, psi1 = 0, psi2 = 0, psi3 = 0, psi4 = 0, psi5 = 0, psi6 = 0
)
...
model.samples <- coda.samples(model.fit, c("pred_home","y_homegoals","pred_away", "y_awaygoals", "beta0","beta1","beta2","beta3","beta4","beta5","beta6", "psi", "dev"), n.iter=iterations)
# model.samples <- coda.samples(model.fit, c("psi", "psi0","psi1","psi2","psi3","psi4","psi5","psi6"), n.iter=iterations)
# model.samples <- coda.samples(model.fit, c("psi", "beta0","beta1","beta2","beta3","beta4","beta5","beta6"), n.iter=iterations)
#------------------------------------------------------------------------------#
...
ci_hdi[which(grepl("psi", ci_hdi$Parameter)),]
...
summary_mean_list[grep("psi", names(summary_mean_list))]
summary_sd_list[grep("psi", names(summary_sd_list))]
...
df_pr <- data %>% select(HomeTeam, AwayTeam) %>% 
  cbind(pred_home, pred_away) %>%
  rename(HomeGoal=pred_home, AwayGoal=pred_away)
row.names(df_pr) <- NULL

df_pr$HomeGoal <- floor(df_pr$HomeGoal)
df_pr$AwayGoal <- floor(df_pr$AwayGoal)

df_score_histogram <- df_pr %>% 
...
ggplot(df_score_histogram, aes(x = Score, fill = ScoreType)) +
  ...
  theme_minimal() +
  labs(title = "ZIP Predicted Home vs Away Scores (21-22)", x = "Score", y = "Frequency") +
  xlim(-0.5, 8)
...
points <- t(apply(df_pr, 1, function(x) calculate_points(x['HomeGoal'], x['AwayGoal'])))
rankdf_pr <- cbind(df_pr, points) 

# Home data and away data
home_agg_data <- rankdf_pr %>%
...
# write.csv(final_observed_data, file = "zip_final_data_2021_epl.csv", row.names = FALSE)
